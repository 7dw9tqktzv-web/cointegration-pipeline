...
## 12. Verification HL intraday — 3 methodes comparees

### Methode 1 : HL zero-crossing (traversees de zero)

HL = 2-3 barres (10-15 min) sur toutes les paires.
**INVALIDE** — mesure le bruit de tick qui flippe de signe, pas la
mean-reversion exploitable.

### Methode 2 : HL AR(1) intraday (parametrique)

Barres empilees des 30 dernieres sessions cointegrees, centrees par session.
OLS : spread_t = phi * spread_{t-1}. kappa = -ln(phi), HL = ln(2)/kappa.

| Paire  | phi    | HL_AR1      | R2     |
|--------|--------|-------------|--------|
| ZC_ZW  | 0.9898 | 67b (336 min)  | 0.9580 |
| GC_SI  | 0.9906 | 73b (365 min)  | 0.9818 |
| NQ_RTY | 0.9944 | 123b (616 min) | 0.9826 |
| YM_RTY | 0.9934 | 105b (523 min) | 0.9777 |
| CL_NG  | 0.9973 | 258b (1292 min)| 0.9782 |

### Methode 3 : HL exploitable (seuil significatif)

Temps pour qu'une excursion |spread_centre| > 1.0 sigma_session revienne
a |spread_centre| < 0.25 sigma_session. C'est le HL tradeable.

| Paire  | HL_expl (med) | P25  | P75  | P90  | Excursions resolues |
|--------|---------------|------|------|------|---------------------|
| ZC_ZW  | 18b (90 min)  | 7b   | 42b  | 60b  | 79/98 (81%)         |
| GC_SI  | 16b (80 min)  | 6b   | 41b  | 68b  | 115/138 (83%)       |
| NQ_RTY | 24b (120 min) | 10b  | 47b  | 69b  | 71/98 (72%)         |
| YM_RTY | 25b (125 min) | 8b   | 48b  | 70b  | 70/97 (72%)         |
| CL_NG  | 41b (205 min) | 12b  | 62b  | 91b  | 71/95 (75%)         |

### N optimal par paire (matche HL_exploitable)

| Paire  | N  | T_limite (P75-based)  |
|--------|----|-----------------------|
| ZC_ZW  | 20 | ~12:00 CT             |
| GC_SI  | 20 | ~12:00 CT             |
| NQ_RTY | 24 | ~11:35 CT             |
...
## 13. Univers tradeable — 5 paires eligibles

### Filtre sizing residual

| Paire  | Residu sizing | Micro dispo    | Verdict      |
|--------|---------------|----------------|--------------|
| ZC_ZW  | 15%           | MZC/MZW        | ELIGIBLE     |
| GC_SI  | 22%           | MGC/SIL        | ELIGIBLE (1) |
| NQ_RTY | 5%            | MNQ/M2K        | ELIGIBLE     |
| YM_RTY | 6%            | MYM/M2K        | ELIGIBLE     |
| CL_NG  | 16%           | MCL/QG         | ELIGIBLE     |
| GC_PA  | 45%           | MGC/- (pas PA) | EXCLUE       |
| CL_HO  | 148%          | MCL/- (pas HO) | EXCLUE       |

(1) GC_SI a 22% en mediane — borderline au seuil 20%. Le residu depend
de beta : 0% quand beta ~0.75, 25% quand beta ~0.10. Inclus car micros
MGC+SIL disponibles et meilleur profil d'excursion (83% resolues, 138
excursions — le plus actif).

---

## 14. Backtest V2.2 — Z intraday + sorties Z-space

### Premier test : TP a Z=0

Z_intraday = (spread - mu_rolling) / sigma_rolling, mu et sigma meme fenetre.
Entree directe a +-2.0, biais directionnel, TP quand Z repasse 0.

| Paire  | N  | Trades | TP  | SL | SC | avgTP  | Sharpe | WR   |
|--------|----|--------|-----|----|----|--------|--------|------|
| ZC_ZW  | 20 | 113    | 112 | 1  | 0  | -$40   | -3.68  | 99%  |
| GC_SI  | 20 | 5      | 5   | 0  | 0  | +$17   | +0.53  | 100% |
| NQ_RTY | 24 | 0      | -   | -  | -  | -      | -      | -    |
| YM_RTY | 24 | 2      | 2   | 0  | 0  | -$118  | -0.44  | 100% |
| CL_NG  | 40 | 0      | -   | -  | -  | -      | -      | -    |

**Probleme identifie** : WR = 99% mais avgTP = -$40. Le TP a Z=0 capture
le lag de mu_rolling (mu rattrape le spread mecaniquement), pas la
mean-reversion reelle. Les "TP" sont des faux positifs.

---

## 15. Backtest V2.2 — sorties spread-space

### Principe

Remplacer les sorties Z-score par des sorties en spread-space avec
references figees au moment de l'entree :
- spread_entry et sigma_entry figes a l'entree
...
### Resultats — ZC/ZW + GC/SI, SL=1.5 sigma_entry

| Paire  | TP     | Trades | TP  | SL  | SC | avgTP  | avgSL  | Sharpe | WR  |
|--------|--------|--------|-----|-----|----|--------|--------|--------|-----|
| ZC_ZW  | 1.0s   | 145    | 106 | 39  | 0  | -$26   | -$84   | -4.22  | 73% |
| ZC_ZW  | 1.5s   | 136    | 91  | 45  | 0  | -$21   | -$84   | -4.00  | 67% |
| ZC_ZW  | 2.0s   | 128    | 77  | 51  | 0  | -$13   | -$83   | -3.82  | 60% |
| GC_SI  | 1.0s   | 7      | 6   | 1   | 0  | +$25   | -$116  | +0.17  | 86% |
| GC_SI  | 1.5s   | 7      | 6   | 1   | 0  | +$44   | -$116  | +0.60  | 86% |
| GC_SI  | 2.0s   | 6      | 5   | 1   | 0  | +$85   | -$129  | +0.95  | 83% |

---

## 16. Diagnostic SL — analyse post-SL et MAE

### MAE sur tous les trades (en sigma_entry)

| Paire  | MAE median | MAE P75  | MAE P90  | MAE max  |
|--------|------------|----------|----------|----------|
| ZC_ZW  | 7.61       | 12.66    | 20.45    | 51.66    |
| GC_SI  | 1.44       | 4.89     | 11.82    | 17.63    |

**ZC/ZW** : MAE median de 7.6 sigma — le spread excurse massivement avant
de revenir. Le SL a 1.5 sigma coupe 83% des trades. Le sigma_entry (sur
20 barres) est trop petit pour representer l'amplitude reelle des
mouvements du spread.

**GC/SI** : MAE median de 1.4 sigma — beaucoup plus coherent. Le sigma
est representatif de la volatilite reelle.

### Post-SL diagnostic — ZC/ZW (51 SL trades)

- **82% des SL reviennent a spread_entry** apres le SL (42/51)
- **61% atteignent le niveau du TP** apres le SL (31/51)
  → ces 31 trades auraient ete gagnants avec un SL plus large
- Retour a spread_entry en median **13 barres** (65 min) apres le SL

### Recommandation SL — ZC/ZW

| SL candidat | Trades touches | % |
|-------------|----------------|---|
| 1.0 sigma   | 111/128        | 87% |
| 1.5 sigma   | 106/128        | 83% |
| 2.0 sigma   | 98/128         | 77% |
| 2.5 sigma   | 96/128         | 75% |
| 3.0 sigma   | 93/128         | 73% |

Meme a SL=3.0 sigma, 73% des trades sont touches. Le probleme n'est pas
le placement du SL — c'est que sigma_entry est structurellement trop petit
pour ZC/ZW. Les excursions reelles sont 5-8x plus grandes que sigma_entry.

---

## 17. Sweep TP 1.0-5.0 sigma — recherche du point de transition

### ZC/ZW — SL 1.5 sigma_entry

| TP     | Trades | TP  | SL  | SC  | SC%  | avgTP  | avgSL  | Sharpe | WR  |
|--------|--------|-----|-----|-----|------|--------|--------|--------|-----|
| 1.0s   | 145    | 106 | 39  | 0   | 0%   | -$26   | -$84   | -4.22  | 73% |
| 1.5s   | 136    | 91  | 45  | 0   | 0%   | -$21   | -$84   | -4.00  | 67% |
| 2.0s   | 128    | 77  | 51  | 0   | 0%   | -$13   | -$83   | -3.82  | 60% |
| 3.0s   | 113    | 51  | 62  | 0   | 0%   | +$4    | -$85   | -3.44  | 45% |
| 4.0s   | 110    | 40  | 70  | 0   | 0%   | +$17   | -$87   | -3.51  | 36% |
| 5.0s   | 108    | 31  | 77  | 0   | 0%   | +$63   | -$86   | -2.16  | 29% |

**Observations :**
- 0% SESSION_CLOSE a tous les niveaux — le SL a 1.5 sigma se declenche
  toujours avant la fin de session
- avgTP devient positif a 3.0 sigma (+$4) — point de break-even des couts
- A 5.0 sigma, avgTP = +$63 mais WR = 29% et 77 SL a -$86
- Le Sharpe reste negatif a tous les niveaux — ZC/ZW non-viable avec
  sigma_entry sur 20 barres

### GC/SI — SL 1.5 sigma_entry

| TP     | Trades | TP  | SL  | SC  | avgTP  | avgSL  | Sharpe | WR  |
|--------|--------|-----|-----|-----|--------|--------|--------|-----|
| 1.0s   | 7      | 6   | 1   | 0   | +$25   | -$116  | +0.17  | 86% |
| 1.5s   | 7      | 6   | 1   | 0   | +$44   | -$116  | +0.60  | 86% |
| 2.0s   | 6      | 5   | 1   | 0   | +$85   | -$129  | +0.95  | 83% |
| 3.0s   | 5      | 4   | 1   | 0   | +$182  | -$129  | +1.00  | 80% |
| 4.0s   | 5      | 4   | 1   | 0   | +$210  | -$129  | +1.00  | 80% |
| 5.0s   | 5      | 4   | 1   | 0   | +$328  | -$129  | +1.04  | 80% |

**Observations :**
- Sharpe positif a TOUS les niveaux, croissant avec le TP
- 1 seul SL sur 5-7 trades (WR 80-86%)
- avgTP croissant : $25 -> $328
- **Mais seulement 5-7 trades sur 3 ans** — statistiquement insuffisant
- Le spread GC/SI a plus d'amplitude en dollars que ZC/ZW, et le sigma_entry
  est representatif de la volatilite reelle (MAE median 1.4 sigma vs 7.6)

---

## 18. Diagnostic beta_Kalman vs beta_OLS

### Resultats (TP=2.0 sigma_entry)

| Paire  | Trades | |diff| mean | |diff| max | |diff|/OLS mean | |diff|/OLS max |
|--------|--------|-------------|------------|-----------------|----------------|
| ZC_ZW  | 128    | 0.0012      | 0.0034     | 0.4%            | 1.6%           |
| GC_SI  | 6      | 0.0019      | 0.0029     | 0.6%            | 0.6%           |

**Verdict** : beta_Kalman et beta_OLS sont quasi-identiques sur ces paires.
La divergence maximale est 1.6% — negligeable pour le sizing.
Le V2.4 (switch sizing OLS vs Kalman selon R2) est inutile sur ces paires.
Le Kalman n'ajoute pas de bruit significatif au sizing.

---

## 19. Bilan V2.2 et probleme structurel

### Ce qui fonctionne

- **Z intraday auto-coherent** : plus de Z a 22 sigma. Le Z est borne
  par construction (mu et sigma meme fenetre)
- **Biais directionnel** : filtre les trades contre la tendance structurelle
- **Sorties spread-space** : references figees a l'entree, pas de faux TP
  par lag de mu_rolling
- **GC/SI** : Sharpe positif (+0.17 a +1.04), WR 80-86%, profitable a
  tous les niveaux de TP

### Ce qui ne fonctionne pas

- **ZC/ZW** : Sharpe negatif a tous les niveaux de TP (-2.16 a -4.22).
  Le sigma_entry (20 barres) est trop petit — MAE median de 7.6 sigma,
  le SL a 1.5 sigma coupe 83% des trades. Les couts ($61 RT) depassent
  les TP sauf a 5.0 sigma ou WR = 29%.
- **NQ_RTY et CL_NG** : 0 trades. Le biais directionnel + cointegration
  blocking eliminient toutes les sessions.
- **Trop peu de trades GC/SI** : 5-7 sur 3 ans. Insuffisant pour conclure.
  Cause : cointegration_blocking sur 437/717 sessions (61%).

### Probleme structurel identifie

sigma_entry (std sur 20 barres) ne scale pas avec l'amplitude reelle des
mouvements du spread sur ZC/ZW. Le ratio MAE/sigma = 7.6 montre que le
sigma est 5-8x trop petit. Pour GC/SI, le ratio est 1.4 — coherent.

Les deux causes possibles :
1. **Le spread ZC/ZW n'est pas stationnaire a l'echelle de 20 barres** —
   il derive localement, ce qui fait que sigma sur 20 barres ne capture
   pas les mouvements de plus grande amplitude
2. **Le ratio multiplier/prix est defavorable** — les couts RT de $61
   representent un mouvement de spread trop grand par rapport a
   sigma_entry, rendant les TP insuffisants

### Pistes

- **Augmenter N** pour ZC/ZW (40, 60 barres) — sigma plus representative
  mais le Z-score sera moins reactif
- **Concentrer les efforts sur GC/SI** — le seul signal structurellement
  coherent, avec le defi de generer plus de trades (relacher MacKinnon,
  augmenter la fenetre de calibration)
- **Investiguer pourquoi NQ_RTY/YM_RTY ne generent pas de trades** —
  le HL_exploitable etait bon (24-25 barres) mais 0 trades en backtest
